...
app.post("/api/send-mail", async () => {
  const html = `
    <h2>ไม่รองรับ Gmail SMTP บน Deno Deploy</h2>
    <p>กรุณาใช้เส้นทาง <code>/api/send-mail-maileroo</code> แทน (Maileroo API)</p>
...
app.post("/api/send-mail-maileroo", async (c) => {
  if (!MAILEROO_API_KEY) {
    const html = `
      ...
      <p>กรุณาตั้งค่า Environment Variables: <code>MAILEROO_API_KEY</code></p>
      <a href="/">กลับหน้าหลัก</a>
    `;
  ...
  if (!MAIL_FROM_ADDRESS) {
    const html = `
      ...
      <p>กรุณาตั้งค่า <code>MAIL_FROM_ADDRESS</code> เช่น <code>no-reply@xxxx.maileroo.org</code></p>
      <a href="/">กลับหน้าหลัก</a>
    `;
    return c.html(html, 500);
  }
